...
string result = authContext.AcquireTokenSilent(_eWSResource, _clientId, UserIdentifier.AnyUser).AccessToken;               
                ...
                try
                {
                    ...
                    result = await cca.AcquireTokenByAuthorizationCode(scope, code).ExecuteAsync();
                    ...
                    //result = await cca.AcquireTokenSilent(scope, accounts.FirstOrDefault()).ExecuteAsync();

                    //var accessToken = result.AccessToken; //Console.WriteLine("Access Token" + result.AccessToken + " "  + accessToken);

                    

                    var UserProfileHandler = new AzureADTokens();

                    try 
                    {
                        // include the user profile information
                        UserProfileHandler.SetUserProfile(result.IdToken,result.AccessToken, context.HttpContext.Session);
                    }
                    catch (Exception ex1)
                    {
